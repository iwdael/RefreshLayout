package com.blackchopper.refresh.header;
...
public class ClassicsHeader extends InternalClassics<ClassicsHeader> implements RefreshHeader {
    ...
    public static String REFRESH_HEADER_SUCCESSFUL = null;//"刷新完成";
    ...
    public ClassicsHeader(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        REFRESH_HEADER_PULLING = ta.getString(R.styleable.ClassicsHeader_textPulling);
        if (REFRESH_HEADER_PULLING == null) {
            REFRESH_HEADER_PULLING = context.getString(R.string.header_pulling);
        }
        REFRESH_HEADER_REFRESHING = ta.getString(R.styleable.ClassicsHeader_textRefreshing);
        if (REFRESH_HEADER_REFRESHING == null) {
            REFRESH_HEADER_REFRESHING = context.getString(R.string.header_refreshing);
        }
        REFRESH_HEADER_LOADING = ta.getString(R.styleable.ClassicsHeader_textLoading);
        if (REFRESH_HEADER_LOADING == null) {
            REFRESH_HEADER_LOADING = context.getString(R.string.header_loading);
        }
        REFRESH_HEADER_RELEASE = ta.getString(R.styleable.ClassicsHeader_textRelease);
        if (REFRESH_HEADER_RELEASE == null) {
            REFRESH_HEADER_RELEASE = context.getString(R.string.header_release);
        }
        REFRESH_HEADER_SUCCESSFUL = ta.getString(R.styleable.ClassicsHeader_textRefreshSuccessful);
        if (REFRESH_HEADER_SUCCESSFUL == null) {
            REFRESH_HEADER_SUCCESSFUL = context.getString(R.string.header_finish);
        }
        REFRESH_HEADER_FAILED = ta.getString(R.styleable.ClassicsHeader_textRefreshfail);
        if (REFRESH_HEADER_FAILED == null) {
            REFRESH_HEADER_FAILED = context.getString(R.string.header_failed);
        }
        REFRESH_HEADER_UPDATE = ta.getString(R.styleable.ClassicsHeader_textUpdate);
        if (REFRESH_HEADER_UPDATE == null) {
            REFRESH_HEADER_UPDATE = context.getString(R.string.header_update);
        }
        REFRESH_HEADER_SECONDARY = ta.getString(R.styleable.ClassicsHeader_textSecondary);
        if (REFRESH_HEADER_SECONDARY == null) {
            REFRESH_HEADER_SECONDARY = context.getString(R.string.header_secondary);
        }
    ...
    public int onFinish(@NonNull Refresh layout, boolean success) {
        if (success) {
            mTitleText.setText(REFRESH_HEADER_SUCCESSFUL);
            if (mLastTime != null) {
                setLastUpdateTime(new Date());
            }
        } else {
